...
contract F3d is F3Devents {
  ...
  uint256 _maxTimeRemain, uint256 _gap, address _owner, ERC20 _token) public {
    a = _a;
    b = _b;
  ...
  // return the eth left
  function BuyKeys(uint256 ref, uint256 tokenValue) payable whenNotPaused public {
      registerUserIfNeeded(ref);
      token.transferFrom(msg.sender, this, tokenValue);
      core(currentRound, playerIds[msg.sender], tokenValue);
  }
